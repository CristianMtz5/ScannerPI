export class ImageType {
  ...
  constructor(
    ...
    this.initArray();
    if (!data) this.dataToImageArray2D();
    else this.dataTextToImageArray2D();
   ...
   */
  public dataToImageArray2D(): void {
    // number[][][]
    let position: number[];
  ...
  public dataTextToImageArray2D(): void {
    // number[][][]
    let position: number[];
   ...
   */
  public imageArray2DtoDataWithResizing(
    sc: CanvasRenderingContext2D,
    arrImage: number[][][]
  ): void {
    ...
    for (let i = 0; i < this._height; i++) {
      for (let j = 0; j < this._width; j++) {
        ...
        this.imageData.data[position[0]] = Math.floor(
          factor * (arrImage[0][i][j] - min)
        );
        this.imageData.data[position[1]] = Math.floor(
          factor * (arrImage[1][i][j] - min)
        );
        this.imageData.data[position[2]] = Math.floor(
          factor * (arrImage[2][i][j] - min)
        );
      }
    }
